...
type NftInfo struct {
    ...
    DeveloperID         uint64  `json:"developer_id" binding:"required"'`
    Edition             uint64  `json:"edition" binding:"required"'`
    DeveloperMetadata   string  `json:"developer_metadata" binding:"required"'`
    ContentURL          string  `json:"content_url" binding:"required"'`
}

// newEmulator returns a emulator object for testing
func newEmulator() *emulator.Blockchain {
    b, err := emulator.NewBlockchain(emulator.WithStorageLimitEnabled(false))
    if err != nil {
        panic(err)
    }
    return b
...
func parseSingleNft(nftRef cadence.Optional) (nftInfo NftInfo) {
    resource := nftRef.Value.(cadence.Resource)

    nftInfo.ID = uint64(resource.Fields[1].(cadence.UInt64))
    nftInfo.DeveloperID = uint64(resource.Fields[2].(cadence.UInt64))
    nftInfo.Edition = uint64(resource.Fields[3].(cadence.UInt64))
    nftInfo.DeveloperMetadata = string(resource.Fields[4].(cadence.String))
    nftInfo.ContentURL = string(resource.Fields[5].(cadence.String))

    return
}

func parseNfts(nftRefs cadence.Array) (nftInfos []NftInfo) {
    for _, ref := range nftRefs.Values {
        nftInfo := parseSingleNft(ref.(cadence.Optional))
